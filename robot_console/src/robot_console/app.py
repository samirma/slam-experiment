...
BANNER = """robot_console {version}  ->  {url}

  W / S   forward / back        Space  stop
  A / D   strafe left / right   + / -  speed
  Q / E   rotate left / right   H      show/hide on-screen hints
                                Esc    quit

Hold a key to drive; the robot stops shortly after you let go.
...
def run(options: Options) -> int:
    ...
    if options.preflight and not preflight(options.host, options.port, timeout=options.preflight_timeout):
        return 2
        ...
        recorder.start(
            {
                ...
                "port": options.port,
                "topics": {
                    "cmd_vel": options.cmd_topic,
                    "odom": options.odom_topic,
                    "camera": options.camera_topic,
                },
                "speed": options.speed,
                "speed_max": options.max_speed,
    ...
    cv2.imshow(WINDOW, draw_overlay(frame, show_help=state.show_help, speed=state.speed))
    ...
    try:
        while state.running:
            ...
            if now - last_status >= 1.0:
                last_status = now
                _print_status(state, odom, latest, link)
...
def _print_status(state: TeleopState, odom: Optional[Odom], latest: LatestFrame, link: RobotLink) -> None:
    command = state.command()
    speed_note = ""
    if state.at_max_speed:
        speed_note = " (max)"
    elif state.speed <= SPEED_MIN + 1e-9:
        speed_note = " (min)"
    camera = f"{latest.rate_hz:4.1f} Hz" if latest.received else "  none"
    pose = f"x={odom.x:+.2f} y={odom.y:+.2f} yaw={odom.yaw:+.2f}" if odom else "no odom"
    sys.stdout.write(
