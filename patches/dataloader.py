...
# Try to import pyrealsense2, but don't fail if missing (for standard webcams)
try:
...
class Webcam(MonocularDataset):
    def __init__(self, cam_id=-1):
        ...
        id_to_open = cam_id if cam_id != -1 else 0
        self.cap = cv2.VideoCapture(id_to_open)

        if not self.cap.isOpened():
            # Attempt fallback to default
            if id_to_open != 0:
                 print(f"Camera {id_to_open} failed to open, trying default 0")
                 self.cap = cv2.VideoCapture(0)

        self.save_results = True

        # Attempt to load calibration
        calib_file = f"calibration_webcam{id_to_open}.yaml"
        if not os.path.exists(calib_file) and os.path.exists("intrinsics.yaml"):
             calib_file = "intrinsics.yaml"
        ...
        if os.path.exists(calib_file):
             ...
             # Assuming yaml structure similar to what Intrinsics.from_calib expects or Main expects
             ...
             if "calibration" in calib_data:
                 ...
                 calibration = calib_data["intrinsics"]
                 if "distortion_coefficients" in calib_data:
                     calibration.extend(calib_data["distortion_coefficients"])

             if calibration:
                 self.use_calibration = True
                 config["use_calib"] = True # Global config override
                 self.camera_intrinsics = Intrinsics.from_calib(
                    self.img_size,
                    w,
                    h,
             ...
             print(f"No calibration found ({calib_file}). Running uncalibrated.")
...
class Intrinsics:
    ...
    def from_calib(img_size, W, H, calib, always_undistort=False):
        ...
        center = True
        if "dataset" in config and "center_principle_point" in config["dataset"]:
            center = config["dataset"]["center_principle_point"]
...
def load_dataset(dataset_path):
    ...
    if "webcam" in split_dataset_type or dataset_path.startswith("webcam"):
        cam_id = -1
        match = re.search(r"webcam(\d+)", dataset_path)
        if match:
            cam_id = int(match.group(1))
        return Webcam(cam_id)
